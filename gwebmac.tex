...
%   * pdfTeX primitives, when producing PDF directly with pdfTeX (\pdfoutput>0);
%   * luatex's \pdfextension forms, when producing PDF with luatex -- luatex has
%     none of the \pdf... primitives, so its back end lives in gwebluapdf.tex,
%     which this file \inputs by itself the moment it sees a luatex PDF run. A
%     web that needs luatex (say, for luamplib figures) thus gets links and
%     bookmarks with nothing to ask for; a Korean web gets them the same way,
%     through kotexgweb.tex's own \input gwebmac.
...
\newif\ifpdf      % native pdfTeX PDF output
\newif\ifdvipdf   % dvipdfmx \special{pdf:...} route
\newif\ifluapdf   % luatex \pdfextension PDF output
\ifx\pdfoutput\undefined\else\ifnum\pdfoutput>0 \pdftrue\fi\fi
\ifx\pdfextension\undefined\else\ifnum\outputmode>0 \luapdftrue\fi\fi
\ifpdf\else\ifluapdf\else\ifx\pdf\undefined\else\dvipdftrue\fi\fi\fi
\newbox\linkbox
\def\dest#1{%
...
% \lapstar (defined below) laps cweave's star past it, outside the link, so the
% star is plain black rather than part of the blue hyperlink, as in cweave.
\def\ss#1{\link{#1}{#1}\lapstar}
\def\sDs#1{\link{#1}{$\underline{\hbox{#1}}$}\lapstar}
...
% Under luatex the four macros just defined are no-ops (neither \ifpdf nor
% \ifdvipdf holds); gwebluapdf.tex restates them with \pdfextension. Loaded
% here, after the defaults, so its definitions win. The \input sits inside the
% conditional, so pdfTeX never reads (nor tokenizes) the Lua-laden file.
\ifluapdf\input gwebluapdf \fi
