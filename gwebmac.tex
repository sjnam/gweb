...
% The \leavevmode comes first, ahead of the group: a paragraph beginning with
% \.{...} would otherwise begin inside it, so \everypar would fire inside the
% group, and a hook that clears itself -- \everypar{\hskip-\parindent
% \everypar{}}, the usual way of unindenting the paragraph after a heading --
% would be restored when the group closed and would go on unindenting the
% paragraphs after it.  \leavevmode boxes nothing, so a long string in text mode
% can still break across lines, as the last line below intends.
\def\.#1{\leavevmode{\tentex
  \def\\{\char92 }\def\{{\char123 }\def\}{\char125 }%
  \def\&{\char38 }\def\^{\char94 }\def\_{\char95 }\def\~{\char126 }%
  \def\${\char36 }\def\#{\char35 }\def\%{\char37 }%
  \let\ =\SP % a control space in a string shows a visible blank, as in cweb
  \ifmmode\hbox{#1}\else #1\fi}} % in math ($$\.{...}$$) box it, so it stays typewriter as in cweb; in text leave it open so a long string can still break
...
\newdimen\commentkern \commentkern=-1.7pt % tighten the "//" comment marker
\def\tagkern{\W} % gap between a struct field's type and its `...' tag; \let\tagkern=\quad (or \def it) to taste
...
% C's "->" (member access through a pointer). Go has no such operator, but prose
% sometimes quotes C; gweave sets a quoted "->" with this symbol, exactly as
% cweave does -- the small lowered arrow of cmmi's \char'176, built as cwebmac's
% \MG is, so a quoted C fragment matches a CWEB document.
\newbox\MGbox
...
\def\vc@lon#1#2{%
  ...
  \dimen@=\ht\tw@   \advance\dimen@  -\dp\tw@   % = 의 (ht-dp)
  \dimen@ii=\ht\z@  \advance\dimen@ii-\dp\z@    % : 의 (ht-dp)
  \advance\dimen@ -\dimen@ii
  \advance\dimen@ .32\ht\tw@
  \raise.5\dimen@\box\z@}
\def\vcentcolon{\mathrel{\mathpalette\vc@lon\relax}}
\def\coloneqq{\mathrel{\vcentcolon\mkern-1.3mu=}}
\catcode`\@=12
...
\def\runhead{%
  ...
  \ifodd\pageno
    \hbox to\pagewidth{\mainfont\sechead\qquad{\Gtitlefont\Gtitle}\hfil
      {\groupfont\grouphead}\qquad\folio}%
  \else
    \hbox to\pagewidth{\mainfont\folio\qquad{\groupfont\grouphead}\hfil
      {\Gtitlefont\Gtitle}\qquad\sechead}%
  \fi}
...
%   * pdfTeX primitives, when producing PDF directly (\pdfoutput>0);
...
%
% luatex is the pdfTeX back end too, indirectly. It has no \pdfoutput and none
% of the \pdf... primitives the branch below calls -- they became spellings of
% luatex's \pdfextension. So, on a luatex PDF run that has not already got a
% \pdfoutput, define exactly those primitives as those spellings and set
% \pdfoutput; then \ifpdf turns true and the pdfTeX branch runs unchanged, no
% separate file needed (as cwebmac does it). Guarded twice: pdftex and tex have
% no \pdfextension, and a run that already supplied \pdfoutput is left alone. A
% non-ASCII outline title must be UTF-16BE, which luatex will not do by itself;
% a localization file adds that (see kotexgweb.tex), so plain \pdfescapestring
% here just passes the bytes through, exactly as luatex writes a title.
\ifx\pdfextension\undefined\else\ifx\pdfoutput\undefined
  \csname newcount\endcsname\pdfoutput % \newcount is \outer in plain TeX
  ...
  \def\pdfescapestring#1{#1}%
  \pdfoutput=1
  \pdfcatalog{/PageMode /UseOutlines}% open the bookmark pane on first view
\fi\fi
...
% Starred forms, for a section a change file touched: only the number is linked;
% \xrefstar sits past it, outside the link, so the star is plain black rather than
% part of the blue hyperlink, as in cweave. \xrefstar is a normal-width star by
% default---in a cross-reference note the number is hugged by the following word
% (``19*절'', ``19*.''), which a zero-width lapping star would collide with. The
% index sets it to \lapstar (below), where the entry is followed by a comma the
% star may lap into, so the columns line up on the digits. This is cweave's
% \let\*=* / \let\*=\lapstar split, kept inside the macro instead of a bare token.
\def\xrefstar{*}
\def\ss#1{\link{#1}{#1}\xrefstar}
\def\sDs#1{\link{#1}{$\underline{\hbox{#1}}$}\xrefstar}
...
% ---- section openers --------------------------------------------------------
% A change file marks the sections it altered: gweave opens such a section with
% \Ms or \Ns (instead of \M or \N), and a star is printed just after its number,
% exactly as cweave does. \changemark is that star, emitted only when \ifsecchanged
% is set and reset immediately, so the number itself stays bare everywhere it is
% used programmatically -- the destination, the running-head mark, the contents
% line. \lapstar laps the star to the right with zero width, hugging the number
% without disturbing anything (cweave's \lapstar).
\newif\ifsecchanged
\def\lapstar{\rlap{*}}
\def\changemark{\ifsecchanged\lapstar\fi\global\secchangedfalse}
\def\Ms#1{\global\secchangedtrue\M{#1}}          % \M for a change-file section
\def\Ns#1#2#3{\global\secchangedtrue\N{#1}{#2}{#3}} % \N for a change-file section
% \maybe/\ifon reproduce cweave's change-only listing: put \let\maybe=\iffalse in
% a change file and only the sections it altered are printed. Each \M/\N opens
% \ifon (on for a changed section, otherwise \maybe) and leaves it open; gweave
% closes it with \fi at the section's end, so an off section's whole body -- its
% heading, commentary, and code -- is skipped by TeX. \maybe is \iftrue by
% default, so with no change file (or without the \let) every section prints.
\newif\ifon
\let\maybe=\iftrue
% \onmaybe must stay a macro, not an inline \let\ifon=\maybe: \ifon and \maybe are
% \let to \iftrue/\iffalse, so \TEX/ skipping the untaken \seton branch would miscount
% them as conditionals. A macro name is skipped harmlessly. (This is cweave's idiom.)
\def\onmaybe{\let\ifon=\maybe}
\def\seton{\ifsecchanged\ontrue\else\onmaybe\fi}
...
\def\M#1{\par\seton
  \ifon\emitdate\vfil\penalty-100\vfilneg\vskip\secskip
  \setmark{#1}\noindent\dest{#1}{\bf #1\changemark.\quad}\rightskip=0pt\ignorespaces}
...
\def\N#1#2#3{\par\seton
  ...
  \noindent\dest{#2}{\bf #2\changemark.\quad #3.\quad}\rightskip=0pt\ignorespaces}
...
% A code part sets its lines at the code font's own leading, not the prose
% baselineskip. The code is Latin 10pt, so plain TeX's 12pt suits it even where the
% prose baselineskip is wider -- kotexgweb sets 15pt to give Hangul prose room, but
% that leaves the (Latin) code looking airy. Tunable in limbo.
\newskip\codebaselineskip \codebaselineskip=12pt
...
\def\BS{\penalty0\hskip .333em plus .167em minus .111em\relax} % block braces/keyword clauses: cweave sets these with its plain interword space (as \W), not a wider one
...
\def\ID#1{\hbox{\it #1\/\kern.05em}} % identifier (\kern as in cwebmac's \\)
\def\KW#1{\hbox{\bf % reserved word/type (\& in cwebmac)
  \def\_{\kern.04em\vbox{\hrule width.3em height .6pt}\kern.08em}%
  #1\/\kern.05em}}
\def\ST#1{\hbox{\tentex #1\kern.05em}} % string/verbatim (cwebmac's \. trails a kern)
\def\STtag#1{\hbox{{\rm`}\tentex #1{\rm`}\kern.05em}} % struct field tag: backquotes kept but set roman, content typewriter
\def\MAC#1{\hbox{\tentex #1\kern.05em}} % const/@d name, typewriter like cwebmac's \.
...
% The small-font roster of change-file sections, printed just before the index
% (gweave emits \ch only when a change file altered something). The numbers come
% ready-joined as \s links (unstarred) -- so it is just a labelled note.
...
% translate gweave's output (the change-file roster \ch too); gweave itself
% needs no change.
...
\def\inx{\ifinx
  ...
  \let\xrefstar=\lapstar        % index entries lap the star into the following comma
  \fi}
...
% @:...@> is typeset by the user macro \9, exactly as in cwebmac. Its default
% supports CWEB's sort-key idiom @:key}{text@>: \9 swallows the key, and the
% {text} that follows it is what prints -- so `@:Konig}{K\H{o}nig, D\'enes@>'
% files under Konig but shows K\H{o}nig, D\'enes. Redefine \9 in limbo to make a
% bare @:text@> visible, e.g. \def\9#1{{\rm #1}} to set it in roman.
\def\9#1{}
\def\IC#1{\9{#1}}
...
\def\beginsecs{\vfil\eject
  \global\def\sechead{\kern-2em}% no section number here; the \kern cancels \runhead's adjacent \qquad so the title sits flush to the margin, as cweave's \nullsec does
