...
\newbox\GPPbox % the "++" symbol
\setbox\GPPbox=\hbox{\kern.5pt\raise1pt\hbox{\sevenrm+\kern-1pt+}\kern.5pt}
\def\GPP{\copy\GPPbox}
\newbox\GMMbox % the "--" symbol
\setbox\GMMbox=\hbox{\kern.5pt\raise1pt\hbox{\sevensy\char0\kern-1pt\char0}\kern.5pt}
\def\GMM{\copy\GMMbox}

% Go's short variable declaration ":=", as one symbol you may redefine. WEB set
% Pascal's ":=" as a left arrow through \K; \GK is the same hook for Go, so a
...
% not". Say, for instance, \let\GAN=\setminus in your limbo to prefer another.
\def\GAN{\oslash}
...
% \ifx\title\Gdefaulttitle is true exactly when none was given.
\let\Gdefaulttitle\title
...
% title in \Ggroupfont; both default to \Gheadfont, but a localization may split
...
\def\Gtitlefont{\Gheadfont}
\def\Ggroupfont{\Gheadfont}
...
\def\Grunhead{%
  ...
  \ifodd\pageno
    \hbox to\Gpagewidth{\mainfont\Gsechead\enspace{\Gtitlefont\Gtitle}\hfil
      {\Ggroupfont\Ggrouphead}\enspace\folio}%
  \else
    \hbox to\Gpagewidth{\mainfont\folio\enspace{\Ggroupfont\Ggrouphead}\hfil
      {\Gtitlefont\Gtitle}\enspace\Gsechead}%
  \fi}
...
\def\pdfURL#1#2{%
  ...
  \else\ifGdvipdf
    \setbox\Glinkbox=\hbox{\special{pdf: bc [ 0 0 1 ]}#1\special{pdf: ec}}%
    \special{pdf: ann width \the\wd\Glinkbox\space height \the\ht\Glinkbox\space
       depth \the\dp\Glinkbox\space << /Subtype /Link /Border [ 0 0 0 ]
       /A << /S /URI /URI (#2) >> >>}\box\Glinkbox\relax
  \else #1 ({\tt#2})\fi\fi}
...
\newwrite\Gtoc
\newtoks\Gtoctoks % holds a starred section's title, unexpanded, for the deferred \write
\newif\ifGtocopen
...
\def\GN#1#2#3{\par\Gemitdate
  ...
  % starred section always begins its own page (\Gsecpagedepth), so no later section
  % overwrites it before then. Only \the\pageno is left for the \write to fill in.
  \Gtoctoks={#3}%
  \write\Gtoc{\string\GTline{#1}{#2}{\the\Gtoctoks}{\the\pageno}}%
...
\def\Gpunct{\penalty0\hskip .1667em plus .09em minus .06em\relax} % after a comma (cweb thinmuskip, 3mu)
\def\Grel{\penalty0\hskip .2778em plus .15em minus .10em\relax} % around a relation (cweb thickmuskip, 5mu)
\def\GW{\penalty0\hskip .333em plus .167em minus .111em\relax} % between two words (cweb's text interword space)
\def\GBS{\penalty0\hskip .5em plus .2em minus .12em\relax} % wider structural space around a block brace (cweb's \5)
\def\GCS{\penalty0\hskip 1em plus .3em minus .1em\relax}  % generous space before a comment (cweb's \5\5\quad)
...
% in \GPB, cweb's ``program brackets'': the code material is math (it uses
% \mathord and friends), so \GPB supplies the enclosing $...$ only when TeX is
...
% cover note). The defaults reproduce \Gcon's own former top and bottom.
