...
called {\it limbo}: it is plain \TeX\ that \cw{gweave} copies to the output (for
example a redefinition of \cw{\bs title}), and that \cw{gtangle} ignores. You do
not write \cw{\bs input gwebmac} yourself --- \cw{gweave} emits it automatically.
...
A section name may itself contain Go code, written between vertical bars, which
\cw{gweave} typesets like code: \cw{@<Update the counts for byte |b|@>} prints as
$\langle\,$Update the counts for byte\/ {\it b}\thinspace$\rangle$. The bars are
part of the name, so the definition and every reference must spell it the same way.
...
\cw{gweave} writes a \TeX\ file that begins with \cw{\bs input gwebmac} (emitted
automatically) and then expects those macros to be available. Make sure \TeX\ can
find \cw{gwebmac.tex}, for example with
...
same but omits the entry from the index. \cw{gweave} underlines the index entry
for the section where an identifier is defined, guessing definitions from
context; write \cw{@!} just before an identifier to force its entry there to be
underlined when the guess would miss it.

A handful of codes adjust the woven layout directly, and are ignored by
\cw{gtangle}: \cw{@,} inserts a thin space, \cw{@/} forces a line break, \cw{@|}
marks an optional line break, and \cw{@\#} forces a line break followed by a
blank line. Since \cw{gweave} mirrors the source rather than reflowing it, CWEB's
prettyprinter hints \cw{@+}, \cw{@[}\thinspace$\ldots$\thinspace\cw{@]}, and
\cw{@;} are unnecessary; they are accepted and ignored.

\section{Customizing the page}
A few macros, redefinable in the limbo of your \cw{.w} file (after \cw{\bs input
gwebmac}), tune the woven pages, just as in CWEB:

\bull \cw{\bs def\bs title\char123 ...\char125} sets the title shown in the
running header and on the contents page; it defaults to the upper-cased file name.

\bull \cw{\bs pagewidth}, \cw{\bs pageheight} (and the column gutter) set the page
size.

\bull To number the pages for inclusion in a larger document, set the contents
page number and the first text page number, e.g.\
\cw{\bs def\bs contentspagenumber\char123 14\char125\bs pageno=15}.

\noindent The header is suppressed on any page for which \cw{\bs iftitle} is true
(\cw{gweave} sets it on the contents page, which is therefore unnumbered).
...
When the woven file is processed with \cw{pdftex}, the section numbers shown in
references, in the index, in cross-reference notes, and on the contents page
become blue hyperlinks: clicking one jumps to that section (and, for the
underlined index entries, to where the identifier is defined). The starred
sections also form a PDF outline (the bookmark tree), nested by their \cw{@*},
\cw{@*1}, \cw{@*2} depths. A plain DVI engine simply omits the links and outline.
