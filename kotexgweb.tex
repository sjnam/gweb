...
% overrides gwebmac wherever Korean text needs it. The blue links and the PDF
% outline already work under luatex through gwebmac itself; the one thing left
% to Korean is UTF-16BE bookmark titles, at the end of this file.
%
...
% ---- Korean PDF bookmarks (UTF-16BE outline titles) -------------------------
% gwebmac already made the blue links and the outline work under luatex (it
% shims the \pdf... primitives; see its "hyperlinks" section). Only the outline
% titles need us: a PDF outline title must be a UTF-16BE string, and luatex
% writes gwebmac's \bookmark argument as raw bytes, so a Korean title would come
% out as mojibake. We convert it with a small Lua helper, gweb_pdfutf16(s),
% which prints s as the body of a PDF literal string: the UTF-16BE byte-order
% mark followed by each code unit as a pair of octal-escaped bytes
% (surrogate-paired above the BMP). ASCII passes through it unharmed.
%
...
\endgroup
\def\bookmark#1#2#3#4{\pdfoutline goto num #2 count #3
