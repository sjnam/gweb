...
\def\GB{\par\nobreak\vskip 3pt\begingroup
  ...
  \hfuzz=2pt \emergencystretch=3em}
\def\GE{\par\endgroup\vskip 3pt}
% One displayed code line: #1 indentation steps, #2 the body, which is a series
% of tight math chunks ($...$) joined by \GS.  Lines that exceed \hsize wrap at
% a \GS, with continuation lines indented one step deeper than the line itself.
\def\GL#1#2{\par
...
\def\GSO{\penalty0\hskip 0pt\relax}                       % @| optional zero-width break
\def\GBL{\par\smallskip}                                  % @# blank line between code lines
